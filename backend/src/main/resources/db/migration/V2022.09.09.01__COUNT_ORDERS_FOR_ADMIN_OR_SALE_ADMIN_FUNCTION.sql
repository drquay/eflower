CREATE OR REPLACE FUNCTION COUNT_ORDERS_FOR_ADMIN_OR_SALE_ADMIN(
...
begin
	if (var_phone_buyer is null) and (var_phone_receiver is null) then
    	select count(*) into totals from orders ord
    	where
        ...
        select count(ord.*) into totals
            from orders ord INNER JOIN customers cus ON ord.buyer_id = cus.id OR ord.receiver_id = cus.id
            where
                (ord.code ilike ('%' || var_code || '%') or var_code is null)
                and
                (ord.status ilike var_status or var_status is null)
